...
interface RankingEntry { id: string; user_id: string; display_name: string; avatar_emoji: string; training_days: number; total_points: number; week_start: string; }

const AVATAR_EMOJIS = ["⚽", "🏆", "🦁", "🔥", "⭐", "🎯", "💪", "🐺", "🦅", "👑", "🏃", "⚡"];

function getWeekStart(): string {
  const now = new Date(); const day = now.getDay(); const diff = now.getDate() - day + (day === 0 ? -6 : 1);
  const monday = new Date(now.setDate(diff)); return monday.toISOString().split("T")[0];
}
...
const Ranking = () => {
  ...
  const fetchRankings = async () => {
    ...
    const { data, error } = await supabase.from("weekly_rankings").select("*").eq("week_start", weekStart).order("total_points", { ascending: false });
    if (!error && data) {
      setRankings(data as RankingEntry[]);
      const mine = (data as RankingEntry[]).find((r) => r.user_id === user.id);
      if (mine) { setUserEntry(mine); setSelectedEmoji(mine.avatar_emoji); }
    }
    setLoading(false);
  };
  ...
  const syncMyScore = async () => {
    if (!user) return;
    const weekStartDate = new Date(weekStart); const weekEnd = new Date(weekStartDate); weekEnd.setDate(weekEnd.getDate() + 7);
    const [{ data: completed }, { data: sessions }, { data: profile }] = await Promise.all([
      supabase.from("completed_trainings").select("completed_at").eq("user_id", user.id).gte("completed_at", weekStartDate.toISOString()).lt("completed_at", weekEnd.toISOString()),
      supabase.from("training_sessions").select("completed_at").eq("user_id", user.id).gte("completed_at", weekStartDate.toISOString()).lt("completed_at", weekEnd.toISOString()),
    ...
    if (userEntry) {
      await supabase.from("weekly_rankings").update({ training_days: trainingDays, total_points: totalPoints, display_name: displayName, avatar_emoji: selectedEmoji }).eq("id", userEntry.id);
    } else {
      await supabase.from("weekly_rankings").insert({ user_id: user.id, week_start: weekStart, training_days: trainingDays, total_points: totalPoints, display_name: displayName, avatar_emoji: selectedEmoji });
    }
    toast.success("Ranking atualizado."); fetchRankings();
  };

  const updateEmoji = async (emoji: string) => {
    setSelectedEmoji(emoji); setShowEmojiPicker(false);
    if (userEntry) { await supabase.from("weekly_rankings").update({ avatar_emoji: emoji }).eq("id", userEntry.id); fetchRankings(); }
  };
  ...
  return (
    <div className="min-h-screen bg-background pb-20 page-enter">
      ...
      <div className="px-4 max-w-md mx-auto -mt-1">
        {/* Avatar picker */}
        <div className="premium-card rounded-2xl p-4 mb-4 animate-fade-in">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="w-11 h-11 rounded-full bg-primary/10 flex items-center justify-center text-xl hover:scale-110 transition-transform">
                {selectedEmoji}
              </button>
              <div>
          ...
          {showEmojiPicker && (
            <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-border/20 animate-fade-in">
              {AVATAR_EMOJIS.map((emoji) => (
                <button key={emoji} onClick={() => updateEmoji(emoji)} className={cn("w-10 h-10 rounded-full flex items-center justify-center text-lg transition-all hover:scale-110", selectedEmoji === emoji ? "bg-primary/20 ring-2 ring-primary" : "bg-surface-2")}>
                  {emoji}
                </button>
              ))}
            </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className={cn("text-sm font-semibold truncate", isMe ? "text-primary" : "text-foreground")}>{entry.display_name} {isMe && "(você)"}</span>
                        <span className="text-sm font-heading font-bold text-foreground ml-2">{entry.total_points} pts</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Progress value={barWidth} className="h-1.5 flex-1" />
                        <div className="flex items-center gap-1">
                          <Flame className="w-3 h-3 text-accent" />
                          <span className="text-[10px] text-muted-foreground font-medium">{entry.training_days}d</span>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="mt-6 premium-card rounded-2xl p-4 animate-fade-in">
          ...
          <div className="space-y-1.5 text-xs text-muted-foreground">
            ...
            <p>Sessão de treino = <span className="font-semibold text-foreground">8 pts</span></p>
          </div>
        </div>
      </div>
      <BottomNav />
