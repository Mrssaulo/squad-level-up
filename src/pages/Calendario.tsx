...
const Calendario = () => {
  ...
  const fetchTrainings = async () => {
    ...
    if (!error && data) {
      setTrainings(
        data.map((d: any) => ({
          ...d,
          exercises_data: d.exercises_data as Exercise[],
        }))
      );
    }
    setLoading(false);
  ...
  const datesWithTrainings = trainings
    .filter((t) => t.status === "pending")
    .map((t) => new Date(t.scheduled_date + "T12:00:00"));

  const selectedDayTrainings = trainings.filter(
    (t) =>
      selectedDate &&
      isSameDay(new Date(t.scheduled_date + "T12:00:00"), selectedDate)
  );
    ...
    navigate("/active-training", {
      state: {
        training: {
          ...
          exercises: training.exercises_data,
        },
        position: "",
        scheduledId: training.id,
      },
    });
  };
  ...
  const openEdit = (training: ScheduledTraining) => {
    ...
    setEditTitle(training.training_title);
    setEditExercises(JSON.parse(JSON.stringify(training.exercises_data)));
  };
  ...
  const saveEdit = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        exercises_data: editExercises as any,
      })
    ...
    <div className="min-h-screen bg-background pb-20">
      <div className="px-4 pt-6 max-w-md mx-auto">
        <div className="flex items-center gap-2 mb-4 animate-fade-in">
          ...
          <h1 className="font-heading text-xl font-bold">Calendário de Treinos</h1>
        </div>

        <div className="gradient-card rounded-xl border border-border/20 p-3 mb-4 animate-slide-up">
          <Calendar
            mode="single"
            ...
            {selectedDayTrainings.length === 0 ? (
              ...
              <div className="space-y-3">
                {selectedDayTrainings.map((training) => (
                  <div
                    key={training.id}
                    className="gradient-card rounded-xl p-4 border border-border/20"
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {training.exercises_data.length} exercícios
                        </p>
                      </div>
                      <span
                        className={cn(
                    ...
                    <div className="space-y-1 mb-3">
                      {training.exercises_data.slice(0, 3).map((ex, i) => (
                        <div
                          key={i}
                          className="flex justify-between text-xs bg-muted/30 rounded-lg px-3 py-1.5"
                        >
                      ...
                      {training.exercises_data.length > 3 && (
                        <p className="text-[10px] text-muted-foreground text-center">
                          +{training.exercises_data.length - 3} exercícios
                        </p>
                      )}
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {editExercises.map((ex, i) => (
                <div
                  key={i}
                  className="p-3 bg-muted/30 rounded-lg space-y-2 relative"
                >
                  <button
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={() =>
                            updateExercise(i, "reps", ex.reps + 1)
                          }
                          className="w-6 h-6 rounded bg-muted flex items-center justify-center"
                        >
                          <Plus className="w-3 h-3" />
            ...
            <Button
              onClick={saveEdit}
              className="w-full h-10 font-heading font-bold bg-primary hover:bg-primary/90"
            >
              Salvar Alterações
            </Button>
          </div>
